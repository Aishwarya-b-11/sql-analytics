--emp, orders, returns data used
--note: To add a column after creating a table use - alter, eg: if your emp table does not have date of brith, then use -> alter
alter table  employee add dob date;
update employee set dob = dateadd(year,-1*emp_age,getdate());

/*1- write a query to print emp name , their manager name and diffrence in their age (in days) for employees, 
whose year of birth is before their managers year of birth*/
--you can also use minus of both the age column, instead of datediff

select e1.emp_name, e2.emp_name as manager_name, e1.dob, e2.dob as manager_dob,  datediff(day,e1.dob,e2.dob) as diff_in_age
from employee e1
inner join employee e2 on e1.manager_id = e2.emp_id
where DATEPART(year,e1.dob)< DATEPART(year,e2.dob)

	-- just to know how self join works, 
	-- dont use e1.emp_id = e2.manager_id -> since I want manager name for empoyee not other way around

select e1.emp_id, e1.emp_name, e1.manager_id, e1.dob, e2.emp_name, e2.dob
from employee e1
inner join employee e2 on e1.manager_id = e2.emp_id
...
write a query to find order ids where there is only 1 product bought by the customer.*/ --i.e order_id with one row, not order_id with multiple rows
...
select order_date, ship_date, datediff(day,order_date, ship_date) - 2 * datediff(week,order_date, ship_date) as no_of_business_days
from orders;

--6- write a query to print 3 columns : category, total_sales and (total sales of returned orders)
	-- the case meaning -> when return is there (not null), then sum the sales otherwise no.
...
--7- write a query to print 3 columns, category, total_sales_2019 (sales in year 2019), total_sales_2020 (sales in year 2020)
...
select top 5 city, avg(datediff(day, order_date, ship_date)) as avg_diff_days
from orders
where region = 'West'
group by city
...
--9- write a query to print emp name, manager name and senior manager name (senior manager is manager's manager)
	--emp_id in select is only for clarity

select e1.emp_name, e2.emp_name as manager_name, e3.emp_name as senior_manager_name, e1.emp_id, e2.emp_id, e3.emp_id
from employee e1
inner join employee e2 on e1.manager_id = e2.emp_id
